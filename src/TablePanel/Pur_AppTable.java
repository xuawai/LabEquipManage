 ...
 public Object[][] getData(List<Pur_App> Pur_Apps){
  if(Pur_Apps.size()>0){
   ...
   for(int i=0;i<Pur_Apps.size();i++){
    ...
    Object[] a={s.getPur_app_id(),s.getLab_id(),s.getPur_app_factory(),s.getPur_app_model(),s.getPur_app_date(),s.getPur_app_price(),s.getPur_app_status(),s.getPur_app_number()};//把List集合的数据赋给Object数组
